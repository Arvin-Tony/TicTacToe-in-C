...
//   a player has won
// Time: O(n)
bool check_win(char x_or_o, char* arr_all_horiz[], char* arr_all_vert[], char arr_diag_1[], char arr_diag_2[]) {
...
void update_diag(char arr_diag_1[], char arr_diag_2[], int row, int col, char x_or_o) {
    assert(arr_diag_1);
    assert(arr_diag_2);
    assert(1 <= row && row <= 3);
    assert(1 <= col && col <= 3);
...
//   play was valid (ie, doesn't overlapp another non-empty space), false otherwise.
...
int main() {
    ...
    printf("Welcome to Tic Tac Toe!\nTo play the game, please enter the position you want to enter your\ncharacter in as two integers: row number column number (no commas or other characters!)\n");

    printf("For example, when prompted, you enter: 1 1\nYour character will print at the top left corner of the board.\n");
    printf("For example, when prompted, you enter: 1 3 \nYour character will print at the top right corner of the board.\n");
    printf("Player 1 is 'X' and Player 2 is 'O'.\n");
    printf("After each turn, the current board will print.\n");
    ...
    while (true) {
        ...
        // must check if two valid numbers provided
        
        /* Will implement later - allowing for commas and other characters
        if (scanf("%d", &row) != 1) {
            printf("Invalid input. Enter a number please.\n");
            scanf(" %c", &garbage);
            continue;
        }
        while (scanf("%d", &col) != 1) {
            printf("Invalid input. Enter a second number as well please:\n");
            scanf(" %c", &garbage);
